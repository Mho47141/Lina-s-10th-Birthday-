...
export const CoverShellScreen: React.FC<CoverShellScreenProps> = ({ lang, onNext }) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center text-white select-none py-1 px-3 sm:px-4 overflow-visible">
      {/* Standalone Mermaid Section - No Frame, No Card Container */}
      <div className="relative z-10 flex flex-col items-center w-full max-w-md sm:max-w-xl overflow-visible">
        {/* Character Stage with Decorative Shape Base Behind & Below */}
        <motion.div
          initial={{ opacity: 0, scale: 0.94, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: 0.7, ease: 'easeOut' }}
          className="relative flex flex-col items-center justify-center w-full pt-2 sm:pt-3 overflow-visible"
        >
          ...
          <img
            ...
            className="absolute -bottom-2 sm:-bottom-3 left-1/2 -translate-x-1/2 w-[95%] max-w-[380px] h-auto object-contain pointer-events-none -z-10 select-none opacity-95"
          />

          {/* Standalone Transparent Mermaid Character: Lina on the Sea Rock */}
          <div
            onClick={handleOpenInvitation}
            className="relative z-10 cursor-pointer flex flex-col items-center w-full overflow-visible"
          >
            <motion.img
              animate={{ y: [0, -4, 0] }}
              ...
              referrerPolicy="no-referrer"
              className="w-auto max-h-[38vh] sm:max-h-[44vh] md:max-h-[48vh] object-contain transition-transform duration-500 hover:scale-[1.02] select-none pointer-events-auto filter drop-shadow-[0_10px_25px_rgba(0,0,0,0.55)]"
            />
            ...
            <motion.button
              ...
              className="absolute bottom-1 -left-1 sm:bottom-3 sm:-left-3 md:-left-5 z-30 w-[84px] h-[84px] sm:w-[98px] sm:h-[98px] rounded-full bg-gradient-to-tr from-cyan-400 via-teal-300 to-pink-400 text-slate-950 flex items-center justify-center border-2 border-white/95 cursor-pointer group select-none shadow-[0_0_30px_rgba(56,189,248,0.7)]"
            ...
            >
              ...
              <span className="font-greatvibes text-2xl sm:text-3xl font-extrabold tracking-normal leading-none text-emerald-950 text-center select-none pt-1">
                Open
              </span>
            </motion.button>
        ...
        <div className="pt-1 text-center relative z-10 flex flex-col items-center overflow-visible">
          <div className="relative overflow-visible filter drop-shadow-[0_6px_25px_rgba(0,0,0,0.85)]">
            <motion.h1
              animate={{ scale: [1, 1.025, 1] }}
              transition={{ repeat: Infinity, duration: 4, ease: 'easeInOut' }}
              className="font-greatvibes text-6xl sm:text-7xl md:text-8xl leading-[1.15] pt-2 pb-1 px-6 overflow-visible text-transparent bg-clip-text bg-gradient-to-r from-cyan-100 via-white to-pink-200 select-none tracking-wide"
            >
              Lina
            </motion.h1>
          </div>
          ...
          <p className="mt-1.5 text-xs sm:text-sm text-cyan-100/95 leading-relaxed font-medium max-w-xs drop-shadow-[0_2px_8px_rgba(0,0,0,0.85)]">
            {lang === 'ar'
